...
> **2026-07-10 선반영 (브라우저 도달 구간)**: 딥링크 공유(검토·프로젝트 "링크 복사"), 백그라운드 동기화(5분 안전망 폴링 + 창 복귀 재조회), 검토요청 INSERT Realtime 즉시 반영(마이그레이션 `202607090001`), 파트장 전용 크롬 데스크톱 알림(벨 패널에서 옵트인). 아래 이메일·메신저 채널은 **브라우저를 닫았을 때의 도달**용으로 남는다.
