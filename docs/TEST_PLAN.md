...
Playwright **preview** project(`npm run test:e2e`)는 local adapter 회귀용 22개 시나리오로 다음 계약을 독립적으로 차단한다: leader/member navigation, command palette, review lifecycle(반려 사유 필수·승인 확인), project CRUD, product transfer, change task lifecycle, common-change final approval, Excel product import, deep-link, density persistence, mobile sidebar, mobile bottom tab bar와 뒤로가기(서랍·창·상세부터 닫기), mobile list→detail, notification/read navigation, review stats filter, modal focus return, responsive topbar·44px 터치 영역.
