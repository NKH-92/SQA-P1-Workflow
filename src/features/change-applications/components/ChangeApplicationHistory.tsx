...
export function ChangeApplicationHistory({
  ...
  useEffect(() => {
    ...
    void fetchPage(filters, null)
      ...
      .catch(() => {
        ...
        setCursor(null)
        setSelectedId(null)
        setError('완료 이력을 불러오지 못했어요. 잠시 후 다시 시도해 주세요.')
      })
      .finally(() => {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      if (sequence !== requestSequence.current) return
      setError('이력을 더 불러오지 못했어요. 잠시 후 다시 시도해 주세요.')
    } finally {
    ...
    <div className="change-history-panel">
      ...
      >
        ...
        <select aria-label="완료 결과" value={draft.result} onChange={(event) => setDraft({ ...draft, result: event.target.value as HistoryForm['result'] })}>
          ...
          <option value="legacy_auto">이전 방식 자동 완료</option>
          <option value="legacy_manual">이전 방식 보관</option>
        </select>
      ...
      {error && (
        <div className="change-history-error" role="alert">
          <AlertCircle size={16} />
          <span>{error}</span>
          <button className="ghost compact" onClick={() => setReloadKey((value) => value + 1)} type="button">다시 시도</button>
        </div>
      )}
      {loading && <p className="change-history-loading" role="status">완료 이력을 불러오고 있어요.</p>}
      {!loading && !error && rows.length === 0 && (
        <EmptyState icon={<History size={22} />} title="조건에 맞는 완료 이력이 없어요" description="검색어나 필터를 바꿔 다시 확인해 주세요." />
      )}
            ...
            {rows.map((row) => (
              ...
              >
                <span className="change-history-main">
                  ...
                  <b>{row.title}</b>
                  <small>{formatDate(row.history_at)} · {row.final_completed_by_name ?? '이전 기록'}</small>
                </span>
          ...
          {selected && (
            <article className="change-history-detail" aria-label="완료 이력 상세">
              ...
              <dl>
                ...
                <div><dt>최종 확인자</dt><dd>{selected.final_completed_by_name ?? '이전 기록'}</dd></div>
              ...
              <div className="change-history-task-list">
                {selected.product_tasks.map((task) => (
                  <div key={task.id}>
                    <span><strong>{task.product_name}</strong><small>{task.assignee_name ?? '담당자 없음'}</small></span>
