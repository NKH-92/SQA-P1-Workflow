...
describe('Supabase migrations', () => {
  ...
  it('adds read-only team leader access and stateful password transitions', () => {
    const enumMigration = readMigration('20260820150507_add_team_leader_role.sql')
    const migration = readMigration('20260820150511_account_admin_and_team_leader_access.sql')

    expect(enumMigration).toContain("alter type public.app_role add value if not exists 'team_leader'")
    expect(migration).toContain('create or replace function public.current_session_is_valid()')
    expect(migration).toContain('create or replace function public.can_manage_team_data()')
    expect(migration).toContain("profile.role in ('leader', 'team_leader')")
    expect(migration).toContain("profile.role = 'leader'")
    expect(migration).toContain('SQA_TEAM_LEADER_READ_ONLY')
    expect(migration).toContain("cls.relname <> 'review_read_receipts'")
    expect(migration).toContain('private.account_password_change_state')
    expect(migration).toContain("mode in ('issue_temp', 'complete_temp')")
    expect(migration).not.toContain('12345678')
    const reviewScopeMigration = readMigration('20260820161242_fix_team_leader_review_rpc_scope.sql')
    expect(reviewScopeMigration).toContain("ctx.role in ('leader', 'team_leader')")
    expect(reviewScopeMigration).toContain("v_role in ('leader', 'team_leader')")
    expect(reviewScopeMigration).toContain('create or replace function public.mark_all_relevant_reviews_seen()')
  })
})
